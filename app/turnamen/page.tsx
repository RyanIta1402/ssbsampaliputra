import type { Metadata } from "next";
import { redirect } from "next/navigation";

import AccountHeader from "@/components/account/AccountHeader";
import TurnamenGrid, { type TurnamenRow } from "@/components/account/TurnamenGrid";
import { canManage, getSession } from "@/lib/auth";
import { getSql, isDbConfigured } from "@/lib/db";

export const metadata: Metadata = {
  title: "Turnamen",
  robots: { index: false, follow: false },
};

export default async function TurnamenPage() {
  const session = await getSession();
  if (!session) redirect("/login");
  if (!canManage(session.role)) redirect("/akun");

  let rows: TurnamenRow[] = [];
  let dbError = false;
  if (isDbConfigured) {
    try {
      const sql = getSql();
      rows = (await sql`
        select id, namaturnamen, keteranganturnamen, tglturnamen,
               created_at, updated_at
        from turnamen
        order by tglturnamen desc nulls last, created_at desc
      `) as TurnamenRow[];
    } catch (err) {
      console.error("Gagal memuat turnamen:", err);
      dbError = true;
    }
  }

  return (
    <main className="min-h-screen bg-ink">
      <AccountHeader role={session.role} userName={session.nama} />
      <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8">
        <p className="tag-label mb-3">Manajemen Master</p>
        <h1 className="font-display text-3xl uppercase leading-tight text-bone sm:text-4xl">
          Data <span className="text-pitch">Turnamen</span>
        </h1>

        {!isDbConfigured && (
          <p className="mt-8 border border-gold/30 bg-gold/10 p-4 text-sm text-gold">
            Database Neon belum dikonfigurasi. Isi DATABASE_URL di .env.local —
            lihat NEON_SETUP.md.
          </p>
        )}
        {dbError && (
          <p className="mt-8 border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-400">
            Gagal memuat data turnamen.
          </p>
        )}

        <div className="mt-8">
          <TurnamenGrid rows={rows} />
        </div>
      </div>
    </main>
  );
}
