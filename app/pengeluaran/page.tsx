...
import PengeluaranGrid from "@/components/account/PengeluaranGrid";
...
export default async function PengeluaranPage() {
  ...
  if (isDbConfigured) {
    try {
      const sql = getSql();
      rows = (await sql`
        select id, tgl, keterangan, nominal, created_at, updated_at
        from pengeluaran
    ...
    <main className="min-h-screen bg-ink">
      ...
      <div className="mx-auto max-w-7xl px-5 py-12 lg:px-8">
        ...
        <h1 className="font-display text-3xl uppercase leading-tight text-bone sm:text-4xl">
          Kas <span className="text-pitch">Pengeluaran</span>
        </h1>
        ...
        {dbError && (
          <p className="mt-8 border border-red-500/30 bg-red-500/10 p-4 text-sm text-red-400">
            Gagal memuat data pengeluaran. Pastikan tabel `pengeluaran` ada di
            database.
          </p>
        )}

        <div className="mt-8">
          <PengeluaranGrid rows={rows} />
        </div>
      </div>
    </main>
  );
